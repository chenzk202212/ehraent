...
def _ensure_ehragent_tools_importable():
    """Prefer EhrAgent/tools over a same-named package in site-packages (venv clash)."""
    if _EHRAGENT_REPO not in sys.path:
        sys.path.insert(0, _EHRAGENT_REPO)
    elif sys.path[0] != _EHRAGENT_REPO:
        try:
            sys.path.remove(_EHRAGENT_REPO)
        except ValueError:
            pass
        sys.path.insert(0, _EHRAGENT_REPO)
    mod = sys.modules.get("tools")
    if mod is not None:
        loc = (getattr(mod, "__file__", None) or "") or ""
        if "site-packages" in loc.replace("\\", "/"):
...
def run_code(cell):
    ...
    try:
        _ensure_ehragent_tools_importable()
        global_var = {"answer": 0}
